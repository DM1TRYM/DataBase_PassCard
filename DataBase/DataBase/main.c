...
void QickSort(Element* left, Element* right, int d, int(*cmp)(const void*, const void*))
{
    if (left == right)
        return;
    else
        ...
        else
        {
            ...
            swap(last, left);
            QickSort(left, last, d, cmp);
            if (last != right)
                QickSort(last->next, right, d, cmp);
        }
}
...
{
    QickSort(list->data, list->tail, d, cmp);
}
